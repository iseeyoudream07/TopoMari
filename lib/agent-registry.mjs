...
export class AgentRegistry {
  constructor(filePath, { reloadIntervalMs = 1000, backupPath = "", logger = console } = {}) {
    this.filePath = resolve(filePath);
    this.backupPath = backupPath ? resolve(backupPath) : "";
    if (this.backupPath === this.filePath) {
      throw new Error("Agent registry backup path must differ from the primary path");
    }
  ...
  async reload(force = false) {
    ...
    if (!force && !primaryMissing && modifiedAt === this.lastModifiedAt) return;

    let config;
    if (primaryMissing && this.backupPath) {
      const backup = await readAgentConfig(this.backupPath);
      if (backup.agents.length) {
        await writeAgentConfig(this.filePath, backup);
        config = backup;
        modifiedAt = (await stat(this.filePath)).mtimeMs;
        this.recoveredAt = new Date().toISOString();
        this.recoveryCount += 1;
        this.logger?.warn?.(
          `Agent registry was missing and has been restored from its runtime backup (${backup.agents.length} agents).`,
        );
      }
    }
    config ||= await readAgentConfig(this.filePath);
    if (this.backupPath && !primaryMissing) {
      await writeAgentConfig(this.backupPath, config);
    }
  ...
  async status() {
    await this.reload();
    return {
      protectedByBackup: Boolean(this.backupPath),
  ...
  async runMutation(callback) {
    const work = async () => {
      // Recover a missing primary before any mutation can create a fresh,
      // partial registry and overwrite the redundant copy.
      ...
      if (this.backupPath) await writeAgentConfig(this.backupPath, config);
      await this.reload(true);
      return result;
    };
