...
export function selectNodeTargetHost(node) {
  const ipv4 = String(node?.ipv4 ?? node?.IPv4 ?? "").trim();
  if (isIP(ipv4) === 4) return ipv4;

  const rawIpv6 = String(node?.ipv6 ?? node?.IPv6 ?? "").trim();
  const ipv6 = rawIpv6.startsWith("[") && rawIpv6.endsWith("]")
    ? rawIpv6.slice(1, -1).trim()
    : rawIpv6;
  return isIP(ipv6) === 6 ? ipv6 : "";
}
...
export class KomariClient {
  ...
  getRecentNodeReports(uuid) {
    const nodeId = encodeURIComponent(String(uuid || "").trim());
    return this.request(`/api/recent/${nodeId}`, { timeoutMs: Math.min(this.timeoutMs, 3_000) });
  }
    ...
    return asArray(candidates)
      .map((node) => {
        ...
        return {
          ...
          targetHost: selectNodeTargetHost(node),
          ipv4: isIP(ipv4) === 4 ? ipv4 : "",
          ipv6: isIP(ipv6) === 6 ? ipv6 : "",
        };
      })
      .filter((node) => node.id);
  }
