...
import { buildDemoKomariOverview, buildKomariOverview } from "./komari-monitor.mjs";
import { TopologyConfigStore } from "./topology-config-store.mjs";
import { sanitizeSiteSettings } from "./topology-config.mjs";
...
function edgeHealthThresholds(config, edge) {
...
export function attachNodeLocations(nodes, locations) {
  const locationMap = locations instanceof Map
    ? locations
...
function virtualNode(descriptor) {
  if (descriptor.id === "client") {
    return {
      ...
      online: true,
      virtual: true,
      ...(Number.isFinite(descriptor.latitude) && Number.isFinite(descriptor.longitude)
        ? { latitude: descriptor.latitude, longitude: descriptor.longitude }
        : {}),
    };
  }
  return {
  ...
  return {
    ...
    ...(live?.countryCode
      ? {
          ...
          countryName: live.countryName || live.countryCode,
          locationSource: "maxmind",
        }
      : {}),
    ...(Number.isFinite(descriptor.latitude) && Number.isFinite(descriptor.longitude)
...
export async function buildLiveDashboard(client, config, { probeStore = null, geoIpService = null } = {}) {
  const site = sanitizeSiteSettings(config);
  ...
  const [nodePayload, taskPayload, nodeLocations] = await Promise.all([
    client.getNodes(),
    hasKomariTasks ? client.getPingTasks() : Promise.resolve([]),
    site.geoIp.enabled && geoIpService?.resolveNodeLocations
      ? geoIpService.resolveNodeLocations().catch(() => new Map())
      : Promise.resolve(new Map()),
  ]);
  const liveNodes = attachNodeLocations(normalizeNodeList(nodePayload), nodeLocations);
  const komariPromise = buildKomariOverview({ nodePayload, client, locations: nodeLocations });
  ...
  async function edgeStats(edge) {
    if (edge.probe_id) {
      ...
      return {
        ...
        stats: probeStore.getEdgeStats(edge.probe_id, config.history_hours || 1, edgeHealthThresholds(config, edge)),
        error: null,
      };
    }
    if (Array.isArray(edge.task_ids) && edge.task_ids.length) {
      ...
      try {
        ...
        const thresholds = edgeHealthThresholds(config, edge);
        const groupStats = groupTasks.map((task) => computeEdgeStats(payload, task.id, thresholds));
        return {
          task: {
          ...
          stats: combineEdgeStats(groupStats, thresholds),
          error: null,
        };
      } catch (error) {
        return {
    ...
    const sourceUuid = String(edge.source_uuid || edge.from || "");
    try {
      return { task, stats: await komariTaskStats(sourceUuid, task, edgeHealthThresholds(config, edge)), error: null };
    } catch (error) {
      return {
        task,
        stats: { latest: null, avg: null, min: null, max: null, loss: null, total: 0, status: "failed", updatedAt: null, history: [] },
...
export function buildDemoDashboard(config) {
  ...
  const routes = config.routes.map((route, routeIndex) => {
    ...
    const edges = route.edges.map((edge, edgeIndex) => {
      ...
      const status = classifyHealth({ hasData: true, loss, avg }, edgeHealthThresholds(config, edge));
      return {
        ...edge,
        task: {
  ...
  return {
    ...
    komari: buildDemoKomariOverview(usedNodes),
  };
}
