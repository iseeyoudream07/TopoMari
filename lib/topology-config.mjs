...
export function sanitizeThemeSettings(value) {
  ...
  return {
    stopGlobeRotation: booleanSetting(source, "stopGlobeRotation", "stop_globe_rotation"),
...
export function mergeThemeSettings(currentValue, requestedValue, visualTheme) {
  const current = sanitizeThemeSettings(currentValue);
  const requested = requestedValue?.themeSettings ?? requestedValue?.theme_settings ?? requestedValue ?? {};
  if (visualTheme === "glassmorphism") {
    return sanitizeThemeSettings({ ...current, ...requested });
  }
  const hasRotationSetting = Object.hasOwn(requested, "stopGlobeRotation")
    || Object.hasOwn(requested, "stop_globe_rotation");
  return {
    ...current,
    stopGlobeRotation: hasRotationSetting
      ? booleanSetting(requested, "stopGlobeRotation", "stop_globe_rotation", current.stopGlobeRotation)
      : current.stopGlobeRotation,
  };
}
